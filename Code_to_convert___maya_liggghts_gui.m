...
for j=1:handles.nBones
    ...
    h = AllJointsm(1,(handles.bonesTXT{j,2}*6)-2);
    a = AllJointsm(1,(handles.bonesTXT{j,2}*6)-1);
    b = AllJointsm(1,(handles.bonesTXT{j,2}*6));
    ...
    norma = sqrt((myr(1)*myr(1))+(myr(2)*myr(2))+(myr(3)*myr(3)));
    
    initialRotationData(1,j) = myr(4)/(pi/180);
    initialRotationData(2,j) = myr(1)/norma;
    initialRotationData(3,j) = myr(2)/norma;
    initialRotationData(4,j) = myr(3)/norma;

    %initialRotationData(:,j)  %uncomment this to get some debugging info.
...
for i=2:handles.nFrames
    for j=1:handles.nBones
        ...
        h = AllJointsm(i,(handles.bonesTXT{j,2}*6)-2) - AllJointsm(i-1,(handles.bonesTXT{j,2}*6)-2);
        a = AllJointsm(i,(handles.bonesTXT{j,2}*6)-1) - AllJointsm(i-1,(handles.bonesTXT{j,2}*6)-1);
        b = AllJointsm(i,(handles.bonesTXT{j,2}*6)) - AllJointsm(i-1,(handles.bonesTXT{j,2}*6));


        outputRotationData(i,1,j,3) = h;
        outputRotationData(i,2,j,3) = 1;  %these three changed below
        outputRotationData(i,3,j,3) = 0;
        outputRotationData(i,4,j,3) = 0;
        outputRotationData(i,1,j,2) = a;
        outputRotationData(i,2,j,2) = 0;
        outputRotationData(i,3,j,2) = 1;
        outputRotationData(i,4,j,2) = 0; 
        outputRotationData(i,1,j,1) = b;
        outputRotationData(i,2,j,1) = 0; 
        outputRotationData(i,3,j,1) = 0;
        outputRotationData(i,4,j,1) = 1;

        %now get local X-axis based on current orientation - added 19/1/15
            currYang = AllJointsm(i-1,(handles.bonesTXT{j,2}*6)-1);
            currZang = AllJointsm(i-1,(handles.bonesTXT{j,2}*6));
            ...
            Ry22 = 1;
            Ry23 = 0;
            Ry31 = 0-sind(currYang);
            Ry32 = 0;
            Ry33 = cosd(currYang);
            Rz11 = cosd(currZang);
            Rz12 = 0-sind(currZang);
            Rz13 = 0;
            Rz21 = sind(currZang);
            Rz22 = cosd(currZang);
            Rz23 = 0;
            ...
            outputRotationData(i,2,j,3) = Ry11*Rz11;
            outputRotationData(i,3,j,3) = Rz21*Ry11;
            outputRotationData(i,4,j,3) = Ry31;
...
for j=1:handles.nBones
    ...
    if(initialRotationData(1,j) ~= 0 && initialRotationData(2,j)+initialRotationData(1,j)+initialRotationData(3,j) ~= 0) %%DOUBLE CHECK THIS, * SHOULD BE + I THINK.
        fprintf(fileIDo2, ' rotate axis %e %e %e angle %f', initialRotationData(2,j), initialRotationData(3,j), initialRotationData(4,j), initialRotationData(1,j)); %% ANGLE NEEDS TO BE DEGREES
...
for i=2:handles.nFrames
    ...
    for j=1:handles.nBones
        for c=3:-1:1
            ...
            if(outputRotationData(i,1,j,c) ~= 0 )  %%DOUBLE CHECK THIS, * SHOULD BE + I THINK.
                fprintf(fileIDoutput, 'fix rotate%s%i%s all move/mesh mesh %s rotate origin %e %e %e axis %e %e %e period %f\n', handles.bonesTXT{j,1}, i, mys, handles.bonesTXT{j,1}, currentLocation(i-1,1,j), currentLocation(i-1,2,j), currentLocation(i-1,3,j), outputRotationData(i,2,j,c), outputRotationData(i,3,j,c), outputRotationData(i,4,j,c), ((360/(outputRotationData(i,1,j,c)))*(1/framerate)));
            else
                %may need dummy line
            end
        end
    ...
    for j = 1:handles.nBones
        ...
        if(outputRotationData(i,1,j,c) ~= 0 ) %%DOUBLE CHECK THIS, * SHOULD BE + I THINK.
            fprintf(fileIDoutput, 'unfix rotate%s%i%s\n', handles.bonesTXT{j,1}, i,mys);
        end
        end
    end
    for j = 1:handles.nBones
